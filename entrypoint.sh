...
initialize_mysql_database() {
  # inicializar o diretorio da database
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    # espera para o mysql iniciar (max 30 segundos)
    timeout=30
    echo -n "Inicializando a database"
    while ! /usr/bin/mysqladmin -u root status >/dev/null 2>&1
    do
      timeout=$(($timeout - 1))
      if [ $timeout -eq 0 ]; then
        echo -e "\nnão pode conectar a database. fechando..."
        exit 1
      fi
      echo -n "."
      sleep 1
...
# permite argumentos para ser usado no mysqld-safe
if [[ ${1:0:1} = '-' ]]; then
  EXTRA_ARGS="$@"
  set --
...
# comportamento padrão é inicializar o mysql-safe
if [[ -z ${1} ]]; then
  listen "127.0.0.1"
  apply_configuration_fixes
  remove_debian_systen_maint_password
